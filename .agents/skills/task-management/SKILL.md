...
**Paths:** Command examples use `.opencode/skills/task-management/` (project-local OpenCode). On a global install, use `~/.config/opencode/skills/task-management/` instead. Subagent definitions belong in `~/.config/opencode/agents/<id>.md` or `.opencode/agents/<id>.md`, not under deprecated `agent/subagents/` paths.
